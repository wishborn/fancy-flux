...
{{--
    Carousel Step Item: A content panel for a single carousel step.

    This is the main content container for each carousel slide.
...
@props([
    'name' => null, // Unique identifier for this step (required)
...
// Content wrapper with default height for barebones usage
...
<div
    ...
    data-name="{{ $stepName }}"
    role="tabpanel"
    :aria-hidden="!isActive('{{ $stepName }}')"
    aria-label="{{ $label ?? $stepName }}"
>
        ...
        @if ($src)
            ...
            @if (Str::startsWith($src, ['http://', 'https://']) || Str::endsWith($src, ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg']))
                <div class="relative w-full h-full min-h-[200px]">
                    <img
                        src="{{ $src }}"
                        alt="{{ $alt ?? $label ?? 'Carousel image ' . $stepName }}"
        ...
        @else
            ...
            <div class="w-full h-full min-h-[200px] flex items-center justify-center p-6">
                <div class="text-center text-zinc-400 dark:text-zinc-500">
                    <div class="text-4xl mb-2">{{ $label ?? $stepName }}</div>
                    <div class="text-sm">{{ $description ?? 'Slide: ' . $stepName }}</div>
                </div>
            </div>
        @endif
    </div>
